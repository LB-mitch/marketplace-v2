...
const Home: NextPage<any> = ({ ssr }) => {
  ...
  useEffect(() => {
    const redirectAddress = collectionRedirect()
    const chainPrefix = router.query.chain
    if (redirectAddress) {
      router.push(`${chainPrefix}/collection/${redirectAddress}`)
    }
  }, [])
  ...
  const collectionRedirect = () => {
    switch (router.query.chain) {
      case 'ethereum':
        return process.env.NEXT_PUBLIC_ETH_COLLECTION_ADDRESS ?? undefined
      case 'sepolia':
        return process.env.NEXT_PUBLIC_SEPOLIA_COLLECTION_ADDRESS ?? undefined
      default:
        undefined
    }
  }
  ...
  if (!collectionRedirect()) {
    return (
      <Layout>
        ...
        >
                    ...
                    >
                      ...
                      >
                        <Img
                          ...
                          src={
                            optimizeImage(topCollection?.image, 200) as string
                          }
                        />
                      </Box>
                    </Box>
          ...
          <Box css={{ my: '$5' }}>
            <Link
              href={`/${marketplaceChain.routePrefix}/collections/trending`}
            >
    ...
    return (
      <Layout>
        <Head />
        <Flex justify="center" align="center" css={{ height: '50vh' }}>
          <LoadingSpinner />
        </Flex>
        <Footer />
      </Layout>
